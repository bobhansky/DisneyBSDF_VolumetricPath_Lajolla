...
inline Real D_metal(Vector3 h, Real ax, Real ay, Frame &frame) {
	Vector3 local_h = to_local(frame, h);
...
// anisotropic version
inline Vector3 sample_visible_normals(const Vector3& local_dir_in, Real ax, Real ay, const Vector2& rnd_param) {
    // The incoming direction is in the "ellipsodial configuration" in Heitz's paper
    if (local_dir_in.z < 0) {
        // Ensure the input is on top of the surface.
